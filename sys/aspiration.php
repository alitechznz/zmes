    ...
    <section class="content-header">
      <h1>
        ...
        $get_id =0;
            if(isset($_GET['xyz'])){
                $get_id = $_GET['id'];
                echo 'Aspiration Information: ('.$_GET['xyz'].')';
            } else {
                echo 'Aspiration Information';
            }
        ?>
            ...
            if(strpos($user_role['Permission'], 'asp_add') !== false) {
                ...
                <div class="box">
                    <div class="box-header with-border">
                        <h4 class="modal-title"><b><span>Add New Aspiration</span></b></h4>
                    </div>
                
                    <div class="box-body">
                    <form class="form-horizontal" method="POST" action="includes/controller.php">
                    <input type="hidden" class="plan" name="type" id="plan_id" value="aspiration">
                    ...
                    echo '
                        ...
                        <div class="form-group">
                            <label class="col-sm-3 control-label">Aspiration name</label>
                            <div class="col-sm-9"> 
                              <div class="date">
                                <textarea type="text" rows="5" class="form-control" placeholder="Aspiration name" name="aspiration"></textarea>
                            </div>
                            </div>
                        </div>
                ...
                <div class="box-header with-border">
                  ...
                  <table id="example" class="display" style="width:100%">
                    ...
                    <tbody>';
                        ...
                        $query = "SELECT * FROM `themetb`, `agendatb` WHERE `page_name` ='aspiration' AND agendatb.ID=themetb.plan_id"; 
                        $result = mysqli_query($conn, $query) or die("Error : ".mysqli_error($conn));
                        $num = 0;
                        while($row = mysqli_fetch_array($result)) {	
                            $num +=1;
                                ...
                                <td>";
                                  if(strpos($user_role['Permission'], 'ag_delete') !== false) {
                                    echo "<button class='btn btn-danger btn-sm deleteaspiration btn-flat' data-id=".$row['theme_id']."><i class='fa fa-trash'></i> Delete</button>";
                                  }
                                echo    
                                "</td>
                            </tr>";
...
$("body").on("click", ".deleteaspiration", function(e){
 // $('.delete').click(function(e){
    e.preventDefault();
    $('#deleteaspiration').modal('show');
    var id = $(this).data('id');
    getRow_theme(id);
  });
